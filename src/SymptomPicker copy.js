...
function SymptomPicker() {
  ...
  return (
    ...
    <NoSsr>
      <div>
        <div >
          <Label {...getInputLabelProps()}>Symptom Picker</Label>
          <InputWrapper ref={setAnchorEl} className={(focused ? 'focused' : '') +" symptomPicker"}>
            {value.map((option, index) => (
              <Tag className="symptom" label={option} {...getTagProps({ index })} />
            ))}
      ...
      <button onClick={printPredictions.bind(this,value)}>Predict Tests</button>
    </NoSsr>
    </div>
  );
}
